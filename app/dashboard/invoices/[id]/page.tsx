...
// // ── INNER COMPONENT (Saari logic yahan hai) ──
...
//               Edit{" "}
//               <span className="text-amber-400">#{store.invoiceNumber}</span>
//             </span>
//           </div>
...
// // ── MAIN EXPORT (Next.js isay dekhega) ──
// export default function EditInvoicePage() {
//   return (
//     <Suspense fallback={<EditLoadingState />}>
//       <EditInvoiceContent />
//     </Suspense>
//   );
// }
...
function EditInvoiceContent() {
  ...
  return (
    <div className="flex flex-col h-[100dvh] w-full overflow-hidden" style={{ background: "#ECEFF1" }}>
      ...
      {/* ── MAIN CONTENT AREA ── */}
      <main className="flex flex-1 min-h-0 w-full overflow-hidden relative">
        <div className={cn("flex-1 overflow-y-auto custom-scrollbar h-full bg-[#ECEFF1]", tab === "edit" ? "block" : "hidden lg:block")}>
          <div className="p-4 sm:p-6 lg:p-10 max-w-3xl mx-auto pb-32">
            <InvoiceForm />
          </div>
        </div>

        <div className={cn("flex-1 overflow-y-auto custom-scrollbar h-full bg-[#DDE3EA] border-l border-black/5", tab === "preview" ? "block" : (showPreviewPanel ? "hidden lg:block" : "hidden"))}>
          <div className="sticky top-0 z-10 flex items-center justify-between px-5 py-3 bg-[#DDE3EA]/90 backdrop-blur-md border-b border-black/5">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.8)]" />
